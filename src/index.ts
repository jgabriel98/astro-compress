...
import { readFileSyncWithRetry, traverseDirectory } from './helpers.js';
...
export default function GabAstroCompress(options: CompressOptions = {}): AstroIntegration {
  ...
  function getUsedConfig(filePath: string) {
    ...
    const configKey =
      fileExtension in compressionConfig
  ...
  > {
    ...
    const handleCompressedResult = (compressedContent: string | Buffer): Result => {
      const contentLength = Buffer.isBuffer(compressedContent)
        ...
        if (compressionConfig.cache?.enabled) {
          cacheManager = new CompressionCacheManagerImpl(
            path.join(
              ...
              compressionConfig.cache?.cacheDir || defaultCacheDir,
            ),
            logger,
          );
      ...
      'astro:build:done': async ({ assets, dir, logger }) => {
        ...
        for (const candidate of candidates) {
          ...
          if (compressionConfig.cache?.enabled) {
            ...
            const cachedFile = await cacheManager.getCachedFile(
              candidate,
              sourceOriginalHash,
              usedConfig,
            );
            ...
            promises.push(
              processFile(candidate, logger).then((result) => {
                if (result.processed) {
                  logger.info(
                    chalk.blue(`Processed ${candidatePrettyPath}`) +
                      chalk.gray(
                        `- Original size: ${result.originalSize} bytes, New size: ${result.newSize} bytes`,
                      ),
                  );
                  ...
                  logger.warn(
                    `${candidatePrettyPath} did not reduce in size. Saving original file to cache.` +
                      chalk.gray(` - size: ${result.originalSize} bytes`),
                  );
                  const compressedContent = fs.readFileSync(candidate);
                  promises.push(
                    cacheManager.saveToCache(
            ...
            promises.push(
              processFile(candidate, logger).then((result) => {
                if (result.processed) {
                  logger.info(
                    `Processed ${candidatePrettyPath} - Original size: ${result.originalSize} bytes, New size: ${result.newSize} bytes`,
                  );
                  processedFiles++;
                } else {
                  skippedFiles++;
